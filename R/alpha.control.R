...
#' @param bvec Vector containing boundary values for the constraints. Defaults to 
...
#' @param norm.type The type of norm used for identifiability of alpha_j 
#' vectors. See \code{\link[base:norm]{norm}} for available norms.  
#' @param ctol Tolerance value on constraints. See details.
#' @param solver The quadratic programming solver to use. See details.
#' @param qp_pars A named list of parameters to be passed to the \code{solver} 
#' function. 
#'
...
#' 
#' @seealso The main function \code{\link{cgaim}}. The
...
alpha.control <- function(Cmat = NULL, bvec = NULL, alpha.start = NULL,
  init.type = c("regression", "random"), norm.type = "1", 
  ctol = 1e-3, solver = "osqp", qp_pars = list())
{
  ...
  pars$init.type <- match.arg(init.type)
